...
df_revisao_importada = pd.read_excel(os.path.join(cwd, path + arquivos_primarios['df_revisao_importada']),
                           ...
                           dtype = tp_dado_arquivos['df_revisao_importada']).applymap(padronizar)
...
df_revisao_nacional = pd.read_excel(os.path.join(cwd, path + arquivos_primarios['df_revisao_nacional']),
                                    ...
                                    dtype = tp_dado_arquivos['df_revisao_nacional']).applymap(padronizar)
...
capacidade_portos = pd.read_excel(os.path.join(cwd, path + arquivos_primarios['capacidade_portos']),
                                  ...
                                  dtype = tp_dado_arquivos['capacidade_portos']).applymap(padronizar)
...
cap_op_portos = pd.read_excel(os.path.join(cwd, path + arquivos_primarios['cap_op_portos']),
                              ...
                              dtype = tp_dado_arquivos['cap_op_portos']).applymap(padronizar)
...
left_outer_join(df_revisao_importada, periodos, left_on = 'DT_REMESSA', right_on = 'PERIODO')
...
left_outer_join(df_revisao_nacional, periodos, left_on = 'PROXY_PERIODO', right_on = 'pk_NOME_PERIODO')
...
left_outer_join(df_revisao, portos, left_on = 'PORTO', right_on = 'PORTO')
...
left_outer_join(wizard_suprimento_faixa,df_revisao, left_on = 'pk_left', right_on = 'pk_right')
...
capacidade_portos = capacidade_portos.applymap(padronizar)
left_outer_join(capacidade_portos, periodos, left_on = 'PERIODO', right_on = 'PERIODO')
left_outer_join(capacidade_portos, az_portos, left_on = 'PORTO', right_on = 'PORTO')
...
left_outer_join(df_leadtime, df_database_fila, left_on = ['Porto','Periodo'], right_on = ['Porto','Periodo'])
...
left_outer_join(df_leadtime, df_portos, left_on = 'Porto', right_on = 'PORTO')
...
left_outer_join(df1_correntes, df_leadtime_transt, left_on = ['PERIODO','proxy'], right_on = ['Periodo','PROXY_2'] )
left_outer_join(df2_correntes, df_leadtime_fila, left_on = ['PERIODO','proxy'], right_on = ['Periodo','PROXY'])
...
wizard_leadtime['Valor'] = wizard_leadtime['Valor'].apply(custom_round)
